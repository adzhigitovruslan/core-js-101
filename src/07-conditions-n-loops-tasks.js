...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  let str = '';
  const bracket1 = isStartIncluded === false ? '(' : '[';
  const bracket2 = isEndIncluded === false ? ')' : ']';
  if (a > b) {
    str = `${bracket1}${b}, ${a}${bracket2}`;
  } else {
    str = `${bracket1}${a}, ${b}${bracket2}`;
  }
  return str;
}
...
function isCreditCardNumber(ccn) {
  const nums = ccn.toString().split('').map((num) => Number(num));
  ...
  for (let i = nums.length - 1; i >= 0; i -= 1) {
    ...
    if (double) {
      digit *= 2;
      if (digit > 9) {
        digit = digit.toString().split('').reduce((prev, curr) => +prev + +curr);
      }
    }
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (!stack.length) {
      ...
      if (BRACKETS_PAIRS[lastElemInStack] === str[i]) {
        stack.pop();
      } else {
        stack.push(str[i]);
      }
...
function getCommonDirectoryPath(pathes) {
  const directories = pathes.map((filename) => filename.split('/'));
  const minDirs = Math.min(...directories.map((dirs) => dirs.length));

  let commonPath = '';
  for (let i = 0; i < minDirs; i += 1) {
    const dir = directories[0][i];
    if (directories.every((dirs) => dirs[i] === dir)) {
      commonPath += `${dir}/`;
    } else {
      break;
    }
...
function evaluateTicTacToePosition(position) {
  ...
  for (let i = 0; i < winningCombinations.length; i += 1) {
    ...
    for (let j = 0; j < combination.length; j += 1) {
